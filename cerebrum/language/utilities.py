...
class LanguageMemoryUtil():
	...
	def write_memory(data, starting_time, ending_time):
		MEM_FILE_PATH = os.path.expanduser("~/Hippocampus/language/memory/" +  str(datetime.date.today()) + ".mem") # Path for mem file
		TSTP_FILE_PATH = os.path.expanduser("~/Hippocampus/language/memory/" +  str(datetime.date.today()) + ".tstp") # Path for tstp file
