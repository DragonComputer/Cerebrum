...
while True: # Loop over the frames of the video
	...
	if cnts:
		...
		if delta_value > (height * width * float(NON_STATIONARY_PERCENTAGE) / float(100)): # If delta value is too much
			...
			mask = numpy.zeros_like(frame) # Create a mask image for drawing purposes (original frame)
			mask_delta = numpy.zeros_like(frame) # Create a mask image for drawing purposes (delta frame)
